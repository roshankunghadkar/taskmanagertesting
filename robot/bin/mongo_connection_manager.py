...
class MongoConnectionManager(object):
    ...
    def connect_to_mongodb_with_auth(self, username, password, dbName, dbHost='localhost', dbPort=27017,
                                     dbMaxPoolSize=10, dbNetworkTimeout=None,
                                     dbDocClass=dict, dbTZAware=False):
        """
        Loads pymongo and connects to the MongoDB host using parameters submitted.
        ...
        print(
            "| Connect To MondoDB | %s | %s | %s | %s | %s | %s |" % (connURL, dbPort, dbMaxPoolSize, dbNetworkTimeout,
                                                                      dbDocClass, dbTZAware))
